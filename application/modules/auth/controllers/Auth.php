...
class Auth extends REST_Controller {
	...
	function loggedin_get(){
		$response = $this->ion_auth->logged_in('admin@admin.com');
	...
	function session_get(){
		...
		$this->response(date('Y-m-d H:s:i',$session['__ci_last_regenerate']), 200);
	}


	function login_post(){
		$response = array( 'status' => '');
		$identity_value = $this->post('username');
		$token = session_id();
		//$this->response($this->ion_auth->login($identity_value,  $this->post('pwd'), $this->post('remember')), 200);
		if ($this->ion_auth->login($identity_value,  $this->post('pwd'), $this->post('remember'))) {
			...
			if($vresult['result']){
				...
				$response['data'] = array (
					'identification' => $vresult['data'][0]['identification'],
					'username' => $vresult['data'][0]['username'],
					'user_id' => $vresult['data'][0]['iduser'],
					'email' => $vresult['data'][0]['email'],
					'first_name' => $vresult['data'][0]['first_name'],
					'last_name' => $vresult['data'][0]['last_name'],
					'company' => $vresult['data'][0]['company'],
					'token' => $vresult['data'][0]['token'],
					'rol_name' => $vresult['data'][0]['description'],
					'rol_code' => $vresult['data'][0]['code'],
					//'last_login' => date('Y-m-d h:i:s', $vresult['data'][0]['last_login']),
					//'time_session' => date('Y-m-d h:i:s', $vresult['data'][0]['time_session'])
				);
				$this->response($response, 200);
			} else {
				$response['code'] = '-1';
			...
			$response['code'] = $errors_array[0]['code'];
	...
	function logout_get() {
		...
		$session = $this->_checksession($this->get('token'));
		if($session == -1){
			$this->response($this->data_error_response('00', 'Error chequeando sesion.'), 500);
		} else {
			if($session == 0){
				if($this->session_model->update($identity, false, $identity)){
	...
	function changepwd_post(){
		...
		if($session == -1){
			...
			if ($session == 0) {
				$identity = $this->post('identification');
				...
				if ($change){
					...
					$response['code'] = $errors_array[0]['code'];
					$response['message'] = $errors_array[0]['msg'];
					$response['data'] = $this->session->userdata('identity');
					$this->response($response, 500);
				}
