...
abstract class AbstractRelationship implements InterfaceRelationship
{
	...
	/**
	 * Class name of the associated models.
	 *
	 * @var string
	 */
	public $class_name;
	 ...
	 *
	 * @param Table $table
	 * @param $models array of models objects
	 ...
	 * @param $model_values_keys -> key(s)/value(s) to be used in query from models which is including
	 * @return void
	 */
	 ...
	 *
	 * @param Model $model The models which holds this association
	 * @param array $attributes Hash containing attributes to initialize the models with
	 * @return Model
	 */
	public function build_association(Model $model, $attributes=array())
	{
	...
	/**
	 * This will load the related models data.
	 *
	 * @param Model $model The models this relationship belongs to
	 */
...
class HasMany extends AbstractRelationship
{
	 ...
	 * <li><b>through:</b> name of a models</li>
	 * </ul>
	 *
	 * @var array
	 */
		...
		if (!isset($this->initialized))
		{
			if ($this->through)
			{
				...
				if (!($through_relationship = $this->get_table()->get_relationship($this->through)))
					throw new HasManyThroughAssociationException("Could not find the association $this->through in models " . get_class($model));
