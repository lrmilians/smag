 ...
 * for every table you have a models for.
 *
 * @package ActiveRecord
 */
class Table
{
	...
	/**
	 * A instance of CallBack for this models/table
	 * @static
	...
	public static function load($model_class_name)
	{
		if (!isset(self::$cache[$model_class_name]))
		{
			/* do not place set_assoc in constructor..it will lead to infinite loop due to
			   relationships requesting the models's table, but the cache hasn't been set yet */
	...
	private function set_setters_and_getters()
	{
		...
		if (!empty($getters) || !empty($setters))
			trigger_error('static::$getters and static::$setters are deprecated. Please define your setters and getters by declaring methods in your models prefixed with get_ or set_. See
