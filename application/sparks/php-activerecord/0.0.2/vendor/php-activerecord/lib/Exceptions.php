...
class ReadOnlyException extends ModelException
{
	 ...
	 * Sets the exception message to show the undefined property's name.
	 *
	 * @param str $class_name name of the models that is read only
	 * @param str $method_name name of method which attempted to modify the models
	 * @return void
	 */
	public function __construct($class_name, $method_name)
	{
		$this->message = "{$class_name}::{$method_name}() cannot be invoked because this models is set to read only";
		parent::__construct();
	}
};
