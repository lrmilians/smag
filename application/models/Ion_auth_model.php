...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			...
			$this->set_error('password_old_not_math');
			return FALSE;
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, users.id as id, password, active, last_login, token, first_name, last_name, description, code')
		                  ->join('groups', 'users.rol_id = groups.id')
			              ->where($this->identity_column, $identity)
		                  ->limit(1)
		...
		$this->trigger_events('post_login_unsuccessful');
		$this->set_error('login_unsuccessful1');
	}
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'rol_name'                => $user->description,
		    'rol_code'                => $user->code,
		    'user_id'              => $user->id, //everyone likes to overwrite id so we'll use user_id
		    'old_last_login'       => date('Y-m-d H:i:s',$user->last_login)
		);
	...
	**/
	...
	public function messages()
	{
		...
		foreach ($this->messages as $message)
		{
			...
			$_output .= $this->message_start_delimiter . $messageLang[1] . $this->message_end_delimiter;
		}
	...
	public function messages_array($langify = TRUE)
	{
		if ($langify)
		{
			...
			{
				$messageLang = $this->lang->line($message) ? $this->lang->line($message) : '##' . $message . '##';
				$_output[$i]['code'] = $messageLang[0];
				$_output[$i]['msg'] = $this->message_start_delimiter . $messageLang[1] . $this->message_end_delimiter;
				$i++;
			}
			return $_output;
		}
	...
	public function errors()
	{
		...
		foreach ($this->errors as $error)
		{
			...
			$_output = $this->error_start_delimiter . $errorLang[1] . $this->error_end_delimiter;
		}
	...
	public function errors_array($langify = TRUE)
	{
		if ($langify)
		{
			...
			{
				$errorLang = $this->lang->line($error) ? $this->lang->line($error) : '##' . $error . '##';
				$_output[$i]['code'] = $errorLang[0];
				if($errorLang[0] == '0504'){
					$_output[$i]['msg'] = $this->error_start_delimiter . $errorLang[1] . $this->config->item('lockout_time', 'ion_auth') / 60 . ' minutos.' . $this->error_end_delimiter;
				} else {
					$_output[$i]['msg'] = $this->error_start_delimiter . $errorLang[1] . $this->error_end_delimiter;
				}
				$i++;
			}
			return $_output;
